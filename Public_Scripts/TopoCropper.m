...
imagemap = append(maindir, '\RawImages\chip10_figure\'); %contains raw files
destmap = append(maindir, '\CroppedImages\Chip10_figure\'); %contains destination files
...
for chip = 1:(size(chips))
    ...
    if test == 0
       ...
       % im_dest = imread(append(replicatemap,'\',DNA));
        im_dest = imread(replicatemap);
        ...
        for index = 1:3
            if index == 1
                ...
                disp(append('Processing ',channel,' now.'));
                 %im_dest = imread(append(replicatemap,'\',DNA));
                 DNA1="Z:\1 BIS\Shared folders\Nikita\Marta\SCREEN\analysis\chip10\RawImages\chip10_figure\Marta_NK_chip10_zstack_dapi_zeromicrom_.tif";
                 im_dest = imread(DNA1);
                im_dest=imrotate(im_dest,rotation_angle);
            end
            if index == 2
                channel = c1;
                disp(append('Processing ',channel,' now.'));
                %im_dest = imread(append(replicatemap,'\',Mito));
                 Mito1="Z:\1 BIS\Shared folders\Nikita\Marta\SCREEN\analysis\chip10\RawImages\chip10_figure\Marta_NK_chip10_zstack_bf_zeromicrom_.tif";
                  im_dest = imread(Mito1);
                im_dest=imrotate(im_dest,rotation_angle);
            end
            if index == 3
                channel = c2;
                disp(append('Processing ',channel,' now.'));
                %im_dest = imread(append(replicatemap,'\',Nephrin));
                 Nephrin1="Z:\1 BIS\Shared folders\Nikita\Marta\SCREEN\analysis\chip10\RawImages\chip2\Marta_NK_chip2_zstack_nephrin_crop_MAX_project.tif";
                  im_dest = imread(Nephrin1);
                im_dest=imrotate(im_dest,rotation_angle);
            end
